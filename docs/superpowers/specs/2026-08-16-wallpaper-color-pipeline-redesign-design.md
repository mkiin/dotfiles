...
apply.sh <image>
  ...
  8. last_wallpaper 書き込み（全工程成功の記録として最後に書く）
...
4. `apply.sh "$(<last_wallpaper)"`
   - 新規有効化モニタは黒なので表示照合を通過し、awww img 実行
...
- output 揃い待ちタイムアウト（5 秒）: ログを残して工程 2 以降へ進む。表示は次の呼び出しの照合で収束する。
- 表示検証の不一致: 1 回だけ押し直し。なお不一致なら MISMATCH をログし、古い画像から色を作らないよう色生成へは進まない。無限リトライはしない（次の契機で収束）。
- matugen 失敗: `--source-color-index 0` での再試行を維持。
- リスナー等の常駐部品は追加しないため、最悪でも pyprland の次のローテーション（interval 30）が収束点になる。
