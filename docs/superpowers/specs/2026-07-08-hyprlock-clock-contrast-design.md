...
| 幅変動対処 | **3文字略記**（実機確認後に変更）: 月 `%b` 大文字（JUL）・曜日 `%a`（Wed）で日付カラム幅をほぼ一定にし、区切り線との隙間を常に安定させる。当初の「最大幅基準（フルスペル維持）」は短い月の余白（最大約110px）が目立ったため差し替え |
...
- **追記（実機確認後の変更）**: フルスペルでは短い月（JULY 等）で区切り線との余白が最大約110pxに広がり不格好だったため、月・曜日を 3 文字略記（`%b` 大文字 / `%a`）へ変更した。日付カラム幅がほぼ一定（略記月の幅差 ±5px 程度）になったため、区切り線・時刻の座標は略記幅を基準に詰め直した。
...
- 日付カラムは 3 文字略記で幅がほぼ一定になり、任意の月・曜日（`LOCK_CLOCK_AT` で再現）で区切り線と重ならず隙間が安定している。
